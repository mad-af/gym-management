<template>
    <div
        ...
        <!-- Header -->
        <div class="flex flex-col justify-between gap-3 border-b border-gray-200 p-4 sm:flex-row dark:border-gray-800">
            <!-- Action buttons -->
            <div class="flex w-full items-center gap-2">
                <!-- Checkbox dropdown -->
                <div class="relative w-full sm:w-auto">
                    <button @click="openDropDown = !openDropDown"
                        class="flex w-full items-center justify-between gap-3 rounded-lg border border-gray-200 p-3 sm:justify-center dark:border-gray-800"
                        :class="openDropDown
                                ? 'text-gray-700 dark:text-white'
                                : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-white'
                            ">
                        <label for="checkboxAll" class="flex cursor-pointer items-center font-medium select-none">
                            <span class="relative">
                                <input type="checkbox" id="checkboxAll" class="tableCheckbox sr-only"
                                    @change="toggleAll" :checked="allChecked" />
                                <span
                        ...
                        <span :class="openDropDown && 'rotate-180'"
                            ...
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M3.83325 5.91699L7.99992 10.0837L12.1666 5.91699" stroke="currentColor"
                                    stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                        </span>
                    </button>
                    <div v-if="openDropDown"
                        class="absolute top-full left-0 z-40 mt-1 w-40 space-y-1 rounded-2xl border border-gray-200 bg-white p-2 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark">
                        <button v-for="option in dropdownOptions" :key="option"
                ...
                <button v-for="(action, index) in actionButtons" :key="index"
                    class="flex h-10 w-full max-w-10 items-center justify-center rounded-lg border border-gray-200 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white">
                    <component :is="action.icon" />
                </button>

                <!-- More options dropdown -->
                <div class="relative w-full max-w-10">
                    <button @click="openMoreOptions = !openMoreOptions"
                    ...
                    <div v-if="openMoreOptions"
                        class="absolute top-full right-0 z-40 mt-1 w-40 space-y-1 rounded-2xl border border-gray-200 bg-white p-2 shadow-theme-lg transition-colors dark:border-gray-800 dark:bg-gray-800">
                        <button v-for="option in moreOptions" :key="option"
                            class="flex w-full rounded-lg px-3 py-2 text-left text-theme-xs font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                            {{ option }}
                        </button>
                    </div>
        ...
        <Simplebar class="max-h-[510px] 2xl:max-h-[605px]" data-simplebar-auto-hide="false">
            <div>
                <div v-for="(item, index) in mails" :key="index"
                    ...
                    <div class="flex w-3/5 items-center sm:w-1/5">
                        ...
                        <div class="flex items-center">
                            <span class="pr-3 text-gray-400" @click="toggleStar(index)">
                                <svg class="fill-current" :class="item.isStarred ? 'hidden' : 'block'" width="20"
                                ...
                                <svg v-if="item.isStarred" width="20" height="20" viewBox="0 0 20 20" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                    ...
                    <div class="w-1/5">
                        <span class="block text-right text-theme-xs text-gray-400">{{ item.time }}</span>
                    </div>
                </div>
            </div>
        </Simplebar>
        ...
        <!-- Pagination -->
        <div
            class="sticky bottom-0 flex items-center justify-between rounded-b-2xl border-t border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-[#171f2f]">
