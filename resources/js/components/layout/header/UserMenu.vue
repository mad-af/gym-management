<template>
    <div class="relative" ref="dropdownRef">
        <button class="flex items-center text-gray-700 dark:text-gray-400" @click.prevent="toggleDropdown">
            <AppImage container-class="mr-3 h-11 w-11 rounded-full" :src="userAvatar.src"
                :placeholder="userAvatar.placeholder" :alt="user?.name || 'Pengguna'" />

            <span class="mr-1 block text-theme-sm font-medium">{{ user?.name || 'Tamu' }}</span>
        ...
        <div v-show="dropdownOpen"
            ...
            <Link href="/logout" method="post" as="button" @click="closeDropdown"
