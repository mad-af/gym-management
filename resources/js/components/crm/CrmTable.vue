                    ...
                    <tr class="border-t border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                        <th class="px-6 py-3 text-left">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Deal ID</span>
                            </div>
                        </th>
                        <th class="px-6 py-3 text-left">
                            <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Customer</span>
                        </th>
                        <th class="px-6 py-3 text-left">
                            <span
                                class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Product/Service</span>
                        </th>
                        <th class="px-6 py-3 text-left">
                            <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Deal Value</span>
                        </th>
                        <th class="px-6 py-3 text-left whitespace-nowrap">
                            <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Close Date</span>
                        </th>
                        <th class="px-6 py-3 text-left">
                            <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Status</span>
                        </th>
                        <th class="px-6 py-3 text-center">
                            <span class="text-theme-xs font-medium text-gray-500 dark:text-gray-400">Action</span>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="order in orders" :key="order.id" class="border-t border-gray-100 dark:border-gray-800">
                        <td class="px-6 py-3.5 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <div @click="toggleChecked(order)"
                                    class="flex h-5 w-5 cursor-pointer items-center justify-center rounded-md border-[1.25px]"
                                    :class="order.checked
                                            ? 'border-brand-500 bg-brand-500 dark:border-brand-500'
                                            : 'border-gray-300 bg-white dark:border-gray-700 dark:bg-white/0'
                                        ">
                                    <svg v-if="order.checked" width="14" height="14" viewBox="0 0 14 14" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path d="M11.6668 3.5L5.25016 9.91667L2.3335 7" stroke="white"
                                            stroke-width="1.94437" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                                <span class="text-theme-sm font-medium text-gray-700 dark:text-gray-400">{{ order.id
                                    }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-3.5 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <div class="flex h-10 w-10 items-center justify-center rounded-full"
                                    ...
                                    <span class="text-xs font-semibold" :class="order.avatarColor">{{ order.initials
                                        }}</span>
                                </div>
                                <div>
                                    <span
                                    ...
                                    <span class="text-theme-sm text-gray-500 dark:text-gray-400">{{ order.customerEmail
                                        }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-3.5 whitespace-nowrap">
                            <span class="text-theme-sm text-gray-700 dark:text-gray-400">{{ order.product }}</span>
                        </td>
                        <td class="px-6 py-3.5 whitespace-nowrap">
                            <span class="text-theme-sm text-gray-700 dark:text-gray-400">{{ order.value }}</span>
                        </td>
                        <td class="px-6 py-3.5 whitespace-nowrap">
                            <span class="text-theme-sm text-gray-700 dark:text-gray-400">{{ order.closeDate }}</span>
                        </td>
                        <td class="px-6 py-3.5 whitespace-nowrap">
                            <span class="rounded-full px-2 py-0.5 text-theme-xs font-medium"
                                :class="getStatusClass(order.status)">{{ order.status }}</span>
                        </td>
                        <td class="px-6 py-3.5 text-center whitespace-nowrap">
                            <svg @click="deleteOrder(order)"
