        ...
        <div class="custom-scrollbar max-w-full overflow-x-auto">
            <div class="min-w-[500px]">
                <div class="flex flex-col gap-2">
                    <div v-for="(item, index) in items" :key="index" @click="toggleChecked(index)"
                        class="flex cursor-pointer items-center gap-9 rounded-lg p-3 hover:bg-gray-50 dark:hover:bg-white/[0.03]">
                        <div class="flex items-start gap-3">
                            <div class="flex h-5 w-5 items-center justify-center rounded-md border-[1.25px]" :class="item.checked
                                    ? 'border-brand-500 bg-brand-500 dark:border-brand-500'
                                    : 'border-gray-300 bg-white dark:border-gray-700 dark:bg-white/0'
                                ">
                                <svg v-show="item.checked" width="14" height="14" viewBox="0 0 14 14" fill="none"
