<template>
    <div>
        <div class="mx-auto w-full max-w-[385px]">
            <h2 class="mb-7 text-center text-title-sm font-bold text-gray-800 dark:text-white/90">
        ...
        <div>
            <div class="mb-10 text-center">
                <div class="relative z-1 mx-auto inline-flex rounded-full bg-gray-200 p-1 dark:bg-gray-800">
                    <span
                        class="absolute top-1/2 -z-1 flex h-11 w-[120px] -translate-y-1/2 rounded-full bg-white shadow-theme-xs duration-200 ease-linear dark:bg-white/10"
                        :class="isMonthly ? 'translate-x-0' : 'translate-x-full'
                            "></span>
                    <button @click="isMonthly = true" :class="[
                    ...
                    <button @click="isMonthly = false" :class="[
                        'flex h-11 w-[120px] items-center justify-center text-base font-medium',
                        !isMonthly
                            ? 'text-gray-800 dark:text-white/90'
            ...
            <div class="gird-cols-1 grid gap-5 sm:grid-cols-2 xl:grid-cols-3 xl:gap-6">
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <ul class="mb-8 space-y-3">
                        <li v-for="(item, index) in starterPack" :key="index"
                    ...
                    <button
                        class="flex w-full items-center justify-center rounded-lg bg-gray-800 p-3.5 text-sm font-medium text-white shadow-theme-xs transition-colors hover:bg-brand-500 dark:bg-white/10 dark:hover:bg-brand-600">
                        Choose Starter
                    </button>
                </div>

                <!-- Pricing item -->
                <div class="rounded-2xl border border-gray-800 bg-gray-800 p-6 dark:border-white/10 dark:bg-white/10">
                    ...
                    <div class="mb-1 flex items-center justify-between">
                        <div class="flex items-end">
                            ...
                            <span class="mb-1 inline-block text-sm text-white/70">
                                /month
                            </span>
                        </div>

                        <span class="text-theme-xl font-semibold text-gray-300 line-through">
                    ...
                    <ul class="mb-8 space-y-3">
                        <li v-for="(item, index) in mediumPack" :key="index"
                            class="flex items-center gap-3 text-sm text-white/80">
                            <CheckLineIcon class="text-success-500" />
                            {{ item }}
                        </li>
                    </ul>
                    <button
                        class="flex w-full items-center justify-center rounded-lg bg-brand-500 p-3.5 text-sm font-medium text-white shadow-theme-xs transition-colors hover:bg-brand-600 dark:hover:bg-brand-600">
                        Choose Medium
                    </button>
                </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <ul class="mb-8 space-y-3">
                        <li v-for="(item, index) in largePack" :key="index"
                            class="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M13.4017 4.35986L6.12166 11.6399L2.59833 8.11657" stroke="#12B76A"
