    ...
    <AdminLayout>
        ...
        >
            <template #header-actions>
                ...
                <Button v-can="'create_membership_package_items'" size="sm" variant="primary" :onClick="openDrawer" className="w-full sm:w-auto" :endIcon="PlusIcon">
                    Tambah Item
                </Button>
            </template>
            <template #actions="{ row }">
                <div class="flex items-center gap-2">
                    <button v-can="'edit_membership_package_items'" @click="editItem(row)" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-500" title="Edit">
                        <PencilIcon class="w-4.5 h-4.5" />
                    </button>
                    <button v-can="'delete_membership_package_items'" @click="deleteItem(row)" class="text-gray-500 hover:text-error-500 dark:text-gray-400 dark:hover:text-error-500" title="Hapus">
        ...
        <Drawer :isOpen="isDrawerOpen" @close="closeDrawer" :title="form.id ? 'Edit Item' : 'Tambah Item'">
            <div class="space-y-6">
                <div>
        ...
        <Drawer :isOpen="isFilterDrawerOpen" @close="isFilterDrawerOpen = false" title="Filter Item Paket">
            <div class="space-y-6">
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-200">Paket</label>
                    <input type="text" v-model="filters.package_id" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm text-gray-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" />
                </div>
            </div>
            <template #footer>
                <div class="flex w-full justify-end gap-3">
...
const currentPageTitle = ref('Membership Package Items');
const isDrawerOpen = ref(false);
const isFilterDrawerOpen = ref(false);
const filters = ref({ package_id: '' });
...
const fetchItems = async () => {
    const { data } = await axios.get('/api/membership-package-items', {
        params: {
...
const deleteItem = async (row: any) => {
    if (confirm('Hapus item ini?')) {
        await axios.delete(`/api/membership-package-items/${row.id}`);
        fetchItems();
    }
};
...
const saveItem = async () => {
    ...
    try {
        ...
        if (form.value.id) {
            ...
            await axios.post('/api/membership-package-items', payload);
        }
        closeDrawer();
        fetchItems();
    } catch (error: any) {
...
const resetFilter = () => (filters.value = { package_id: '' });
const handleFilter = () => {
    currentPage.value = 1;
    fetchItems();
