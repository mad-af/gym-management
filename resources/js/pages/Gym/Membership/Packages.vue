    ...
    <AdminLayout>
        ...
        >
            <template #header-actions>
                ...
                <Button v-can="'create_membership_packages'" size="sm" variant="primary" :onClick="openDrawer" className="w-full sm:w-auto" :endIcon="PlusIcon">
                    Tambah Paket
                </Button>
            </template>
            <template #actions="{ row }">
                <div class="flex items-center gap-2">
                    <button v-can="'edit_membership_packages'" @click="editItem(row)" class="text-gray-500 hover:text-brand-500 dark:text-gray-400 dark:hover:text-brand-500" title="Edit">
                        <PencilIcon class="w-4.5 h-4.5" />
                    </button>
                    <button v-can="'delete_membership_packages'" @click="deleteItem(row)" class="text-gray-500 hover:text-error-500 dark:text-gray-400 dark:hover:text-error-500" title="Hapus">
        ...
        <Drawer :isOpen="isDrawerOpen" @close="closeDrawer" :title="form.id ? 'Edit Paket' : 'Tambah Paket'">
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" id="name" v-model="form.name" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm text-gray-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" />
                ...
                <div>
                    ...
                    <input type="number" id="price" v-model.number="form.price" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm text-gray-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" />
                ...
                <div>
                    ...
                    <input type="number" id="duration_days" v-model.number="form.duration_days" class="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm text-gray-700 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" />
                    <p v-if="form.errors.duration_days" class="mt-1 text-sm text-error-500">{{ form.errors.duration_days }}</p>
                </div>
                <div class="flex items-center gap-2">
...
const columns = ref<Column[]>([
    { key: 'name', label: 'Nama Paket', sortable: true, class: 'min-w-[200px] font-medium' },
    { key: 'price', label: 'Harga', class: 'min-w-[120px]' },
    { key: 'duration_days', label: 'Durasi (hari)', class: 'min-w-[140px]' },
    { key: 'is_active', label: 'Status', type: 'status', class: 'min-w-[120px]', statusMap: { Active: 'bg-success-50 text-success-700 dark:bg-success-500/15 dark:text-success-500', Inactive: 'bg-error-50 text-error-700 dark:bg-error-500/15 dark:text-error-500' } },
