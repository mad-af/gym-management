    ...
    <AdminLayout>
        ...
        <DynamicTable :columns="columns" :data="tableData" :items-per-page="perPage" :total-items="totalItems"
            ...
            <template #header-actions>
                ...
                <Button v-can="'create_products'" size="sm" variant="primary" :onClick="openDrawer"
        ...
        <Drawer :isOpen="isDrawerOpen" @close="closeDrawer" :title="form.id ? 'Edit Produk' : 'Tambah Produk'">
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" v-model="priceText" inputmode="numeric" @focus="handlePriceFocus"
            ...
            <div class="space-y-6">
                <div>
                    <label class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-200">
                        <input type="checkbox" v-model="filters.only_active"
...
const columns = ref<Column[]>([
    { key: 'name', label: 'Produk', sortable: true, type: 'cover', avatarField: 'cover', labelField: 'name', class: 'min-w-[240px]' },
...
const editItem = (row: any) => {
    form.value = { id: row.id, name: row.name, price: Number(String(row.price).replace(/\D/g, '')) || 0, stock: row.stock === '-' ? 0 : row.stock, errors: {}, processing: false };
    priceText.value = formatRupiah(form.value.price);
    coverFile.value = null;
...
const saveItem = async () => {
    ...
    try {
        ...
        if (form.value.id) {
            await axios.post(`/api/products/${form.value.id}?_method=PUT`, payload, {
