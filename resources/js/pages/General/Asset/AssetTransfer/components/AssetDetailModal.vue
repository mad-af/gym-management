<template>
    <div class="w-full h-full flex justify-center items-center">
        <button v-can="ASSET_PERMISSIONS.VIEW" type="button" @click="open"
...
const fetchData = async () => {
    ...
    loading.value = true;
    try {
        const [assetResponse, extraResponse] = await Promise.all([
            axios.get(`/api/assets/${props.assetId}`),
            axios.get(`/api/assets/${props.assetId}/additional-info`),
        ]);
...
const fetchOptions = async () => {
    try {
        ...
        console.error('Error fetching asset options for modal:', error);
    }
};
