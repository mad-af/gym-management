  ...
  <div>
    ...
    <Drawer :isOpen="isOpen" title="Visits / Check In" @close="closeDrawer">
      <div class="space-y-6">
        ...
        <nav class="flex overflow-x-auto rounded-lg bg-gray-100 p-1 dark:bg-gray-900">
          <button type="button" @click="setActiveTab('member')" :class="[
          ...
          <button type="button" @click="setActiveTab('visitor')" :class="[
            'inline-flex flex-1 items-center justify-center rounded-md px-3 py-2 text-sm font-medium transition-colors duration-200 ease-in-out',
            activeTab === 'visitor'
              ? 'bg-white text-gray-900 shadow-theme-xs dark:bg-white/[0.03] dark:text-white'
              : 'bg-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200',
        ...
        <div v-if="activeTab === 'member'" class="space-y-6">
          <div class="space-y-2">
            ...
            <div class="flex items-start gap-3">
              <div class="shrink-0">
                <Button variant="outline" :onClick="toggleScanner" :disabled="scannerBusy || !scannerSupported">
                  {{ isScannerOpen ? 'Matikan Kamera' : 'Buka Kamera' }}
                </Button>
              </div>
              <div class="min-w-0">
                <p v-if="!scannerSupported" class="text-xs text-gray-500 dark:text-gray-400">
            ...
            <div v-if="isScannerOpen" class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
              <video ref="videoRef" class="aspect-video w-full rounded-md bg-black"></video>
            </div>
          </div>
          ...
          <div class="space-y-2">
            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-200">Kode Member</label>
        ...
        <div v-else class="space-y-6">
          <div class="space-y-2">
            ...
            <Combobox v-model="visitorCustomerId" :options="customerOptions" labelKey="name" valueKey="id"
        ...
        <div v-if="lookupLoading"
          class="rounded-lg border border-gray-200 p-3 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-300">
          {{ activeTab === 'member' ? 'Mencari member...' : 'Memuat customer...' }}
        </div>
        <div v-else-if="foundCustomer"
          class="flex items-center gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-700">
          <div class="h-12 w-12 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
            <AppImage v-if="foundCustomer.avatar" :src="foundCustomer.avatar.url" :alt="foundCustomer.name"
              containerClass="h-12 w-12 rounded-full" imgClass="rounded-full" />
            <UserCircleIcon v-else class="h-6 w-6 m-3 text-gray-400 dark:text-gray-500" />
          </div>
          <div class="min-w-0">
            ...
            <p class="truncate text-xs text-gray-500 dark:text-gray-400">
              {{ foundCustomer.code || foundCustomer.qr_code || '-' }}
            </p>
          </div>
        </div>
        <div v-else-if="activeTab === 'member' && form.qr_code && !lookupLoading"
...
const startScanner = async () => {
  ...
  try {
    ...
    scanLoopActive = true;
    const loop = async () => {
      if (!scanLoopActive || !videoRef.value || !detector) return;
      try {
        const codes = await detector.detect(videoRef.value);
        if (Array.isArray(codes) && codes.length && codes[0]?.rawValue) {
...
const submitDisabled = computed(() => {
  ...
  if (activeTab.value === 'visitor' && (form.value.price === null || Number.isNaN(Number(form.value.price)))) return true;
  return false;
});
...
const submit = async () => {
  ...
  try {
    const payload: any = {
      customer_id: foundCustomer.value?.id || form.value.customer_id || null,
      ...
      qr_code: activeTab.value === 'member' ? (form.value.qr_code || null) : null,
    };
...
onMounted(() => {
  scannerSupported.value = typeof (window as any).BarcodeDetector !== 'undefined' && !!navigator.mediaDevices?.getUserMedia;
  setActiveTab('member');
});
</script>
