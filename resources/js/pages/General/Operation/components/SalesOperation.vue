<template>
    <div>
        <OperationActionButton
            :icon="BanknoteIcon"
            title="Sales"
            description="Input transaksi penjualan."
                        ...
                        >
                            ...
                            <div class="space-y-2">
                                ...
                                <Combobox
                                    ...
                                    :options="getProductOptionsForItem(item)"
                                    labelKey="name"
                                    valueKey="id"
                                    ...
                                    @change="(v) => onProductChange(item, v)"
                                />
                                <p
                                    v-if="errors[`items.${idx}.product_id`]"
                                    class="mt-1 text-sm text-error-500"
                                >
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div class="space-y-2">
                                    ...
                                    />
                                    <p
                                        v-if="getItemStock(item) !== null"
                                        class="text-xs text-gray-500 dark:text-gray-400"
                                    >
                                        Stok tersedia: {{ getItemStock(item) }}
                                    </p>
                                    <p
                                        v-if="errors[`items.${idx}.quantity`]"
                                        class="mt-1 text-sm text-error-500"
                                    >
                                ...
                                <div class="space-y-2">
                                    ...
                                    <p
                                        v-if="getItemUnitPrice(item) !== null"
                                        class="text-xs text-gray-500 dark:text-gray-400"
                                    >
                                        {{
                                            formatCurrencyId(
...
const onProductChange = (item: any, productId: any) => {
    ...
    const prod =
        productOptions.value.find((p: any) => p.id === productId) || null;
    item.product = prod;
};
...
const getItemUnitPrice = (item: any): number | null => {
    const prod =
        item?.product ||
        productOptions.value.find((p: any) => p.id === item?.product_id);
...
const getProductOptionsForItem = (item: any) => {
    ...
    return productOptions.value.filter(
        (p: any) => !chosen.has(p.id) || p.id === item.product_id,
    );
};
...
const submit = async () => {
    ...
    try {
        ...
        form.value.items.forEach((it, idx) => {
            if (!it.product_id) {
                localErrors[`items.${idx}.product_id`] =
                    'Produk wajib dipilih.';
            } else {
                const pid = String(it.product_id);
                if (seenProducts.has(pid)) {
                    localErrors[`items.${idx}.product_id`] =
                        'Produk sudah dipilih di item lain.';
                } else {
            ...
            if (!qty || Number.isNaN(qty) || qty < 1) {
                ...
                if (stock !== null && qty > stock) {
                    localErrors[`items.${idx}.quantity`] =
